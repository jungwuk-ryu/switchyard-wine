    ...
    for (type = 0; type < ARRAY_SIZE(format->f.caps); ++type)
    {
                ...
                && (gl_info->quirks & WINED3D_QUIRK_BROKEN_FBO_DRAW_PROBE))
        {
            TRACE("Skipping format %s FBO blending draw probe, type %u.\n",
                    debug_d3dformat(format->f.id), type);
            /* An unsafe optional probe does not invalidate the format table's
             * statically declared blending capability. */
        }
        ...
        const struct wined3d_vk_info *vk_info, VkPhysicalDevice vk_physical_device)
{
    ...
    vulkan_formats[] =
    {
        ...
        {WINED3DFMT_L8A8_UNORM,                 VK_FORMAT_R8G8_UNORM,              "XXXY"},
        ...
        {WINED3DFMT_L16_UNORM,                  VK_FORMAT_R16_UNORM,               "XXX1"},
        ...
        {WINED3DFMT_L8_UNORM,                   VK_FORMAT_R8_UNORM,                "XXX1"},
