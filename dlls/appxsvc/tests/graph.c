...
#define appx_package_graph_create(snapshot, ...) \
    p_appx_package_graph_create((snapshot), store_root, __VA_ARGS__)
...
static void test_basic_graph( void )
{
    ...
    hr = appx_package_graph_create(
        ...
        loader_files, ARRAY_SIZE(loader_files), &graph );
...
static void test_missing_duplicates_and_loader_input( void )
{
    ...
    {
        {L"App", L"bin\\one.exe", L"", APPX_CATALOG_ACTIVATION_FULL_TRUST},
        {L"App", L"bin\\two.exe", L"", APPX_CATALOG_ACTIVATION_WIN32},
    };
    struct appx_graph_loader_file loader_files[2];
    ...
    ok( hr == APPX_E_INVALID_PACKAGING_LAYOUT,
        "duplicate package basename returned %#lx.\n", hr );
    appx_catalog_snapshot_free( snapshot );
...
static void test_cycles_diamond_and_order( void )
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        check_package( graph, 2, packages[2].full_name, 2 );
...
static void test_corrupt_blob( void )
{
    ...
    {
        {L"Contoso.Main_1.0.0.0_x64__contosoid", L"main.dll"},
        {L"Contoso.FrameworkA_1.0.0.0_x64__contosoid", L"main.dll"},
    };
    struct appx_catalog_package packages[2];
    APPX_CATALOG_SNAPSHOT *snapshot;
    APPX_PACKAGE_GRAPH *graph = NULL;
    ...
    UINT32 size, strings_offset, loader_offset, package_count;
    UINT32 string_ref, original;
    HRESULT hr;
...
#define LOAD(name) p_##name = (void *)GetProcAddress( module, #name )
...
#undef LOAD
    return p_appx_catalog_snapshot_create && p_appx_catalog_snapshot_free &&
           p_appx_package_graph_create && p_appx_package_graph_from_blob &&
