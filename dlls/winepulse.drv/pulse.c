...
static NTSTATUS pulse_create_stream(void *args)
{
    ...
    if (params->spatial)
    {
        WARN("Spatial audio transport with %u dynamic objects at endpoint generation %u "
                "is not supported by the PulseAudio backend.\n",
...
static NTSTATUS pulse_release_render_buffer(void *args)
{
    ...
    /* PulseAudio rejects spatial streams at creation, so the optional spatial
...
static NTSTATUS pulse_wow64_release_render_buffer(void *args)
{
    struct
    {
        stream_handle stream;
    ...
    pulse_release_render_buffer(&params);
    params32->result = params.result;
    return STATUS_SUCCESS;
}
...
const unixlib_entry_t __wine_unix_call_wow64_funcs[] =
{
    ...
    pulse_wow64_release_render_buffer,
