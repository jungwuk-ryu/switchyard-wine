...
The font set supplies regular faces for every Noto family referenced by Wine's DirectWrite fallback table, common bold faces, symbols, a static default-instance `Noto Emoji` outline face, and the Japanese, Korean, Simplified Chinese, Traditional Chinese, and Hong Kong faces from Noto Sans CJK. The emoji face is generated reproducibly from the pinned Noto Emoji variable font by removing its variable-font tables and metadata; it avoids the variable and color-font paths that Wine cannot safely load during prefix setup. The files are installed in `share/wine/fonts` and a relocatable Fontconfig fragment indexes that directory, so both DirectWrite and GDI can discover the runtime-owned faces without copying fonts into `C:\\Windows\\Fonts`. Their pinned URLs, source and generated SHA-256 values live in `switchyard/font-assets.tsv` and the builder; SIL Open Font License 1.1 notices are copied into the runtime.
